...
const handleLogin = () => {
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
        <Card className="w-full max-w-md shadow-2xl">
          <CardContent className="space-y-6">
            <h1 className="text-2xl font-bold text-center">Login: Rischis Kiosk</h1>
            <div>
              <Label>Dein Name</Label>
              <Select value={loginName} onValueChange={setLoginName}>
                <SelectTrigger>
                  <SelectValue placeholder="Namen wählen..." />
                </SelectTrigger>
                <SelectContent>
                  {users.map((u) => (
            ...
            <Button onClick={handleLogin} className="w-full text-lg">Einloggen</Button>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-4">
      <Card className="w-full max-w-md shadow-2xl">
        <CardContent className="space-y-6">
          <h1 className="text-2xl font-bold text-center">Hallo, {user.name}!</h1>
          <p className="text-center">Guthaben: {parseFloat(user.guthaben).toFixed(2)} €</p>
